...
trait UserTrait
{
     ...
     * 
     */
    public function _getAllDistributors(int $uid, int $page, int $num) : array
    {
        $statment = $this->mysqlDao->table("member")
                         ->select(["id as uid", "username", "level", "lastUid", "create_time"])
                         ->where(["lastUid"])
                         ->orderBy(["id" => -1])
                         ->limit($page, $num)
                         ->prepare()
    ...
    public function getUserAddress(int $uid, bool $default = false)
    {
        ...
        return $default ? $addresses[0] : $addresses;
    }
     ...
     * 
     */
    public function updateLastUserReturn(string $orderMoney) : bool
    {
    ...
    public function getMemberList(array $filters, bool $count = false) : array
    {
        ...
        foreach( ["status", "id", "level"] as $option ) {
