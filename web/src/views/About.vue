...
export default class About extends Vue {
    ...
    public init(): void {
      ...
      for ( let i = 0; i < this.table.length; i += 5 ) {
        ...
        let number: any = document.createElement( 'div' );
      ...
      this.controls.addEventListener( 'change', this.renders );

      let button1: any = document.getElementById( 'table' );
      button1.addEventListener( 'click', ( event: any ) => {

        this.transform( this.targets.table, 2000 );

      }, false );

      let button2: any = document.getElementById( 'sphere' );
      button2.addEventListener( 'click', ( event: any ) => {

        this.transform( this.targets.sphere, 2000 );

      }, false );

      let button3: any = document.getElementById( 'helix' );
      button3.addEventListener( 'click', ( event: any ) => {

        this.transform( this.targets.helix, 2000 );

      }, false );

      let button4: any = document.getElementById( 'grid' );
      button4.addEventListener( 'click', ( event: any ) => {
      ...
      container.addEventListener( 'click', this.elementClick, false );

    }

    public transform( targets: any, duration: any ): void {
      ...
      new TWEEN.Tween( this )
        .to( {}, duration * 2 )
        .onUpdate( this.renders )
        .start();

    }

    public elementClick(event: any): void {
      if(event.target.className == 'element'){
        this.handleElement(event.target);
      }
      if(event.target.parentElement.className == 'element') {
        this.handleElement(event.target.parentElement);
      }
    }

    public handleElement(element: any): void {
      console.log(element.querySelector('.number').innerText)
    }
